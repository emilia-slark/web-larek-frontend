...
export function renderBasketItems(items: IProduct[], events: EventEmitter, template: HTMLTemplateElement): HTMLElement[] {
  return items.map((item, index) => {
    let card: CardBasketView;
    card = new CardBasketView({
      container: cloneTemplate(template),
      actions: {
        onClick: () => events.emit('basket:items:remove', { item, card })
      }
    });
    card.index = index + 1;
